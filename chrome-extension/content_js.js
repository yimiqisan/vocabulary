...
(function() {
	var d = !0,
		...
		D = function() {
			chrome.extension.sendMessage({
				...
				this.ia = a.cloneNode(g);           //LZY
				this.ia.id = "gdx-bubble-add";      //LZY
				...
				this.b.appendChild(this.ia);			//LZY
		...
		J = function(a) {
			...
			a: if (b = new H(p, w, p + b, w + c), b.top < document.body.scrollTop) b = g;
				...
				for (var c = document.getElementsByTagName("embed"), I = document.getElementsByTagName("object"), x = [self.pageXOffset, self.pageYOffset], v = 0, N = c.length + I.length; v < N; v++) {
					...
					if (b.bottom > n.top && n.bottom > b.top && b.left < n.right && n.left < b.right) {
						b = g;
						break a
					}
				}
	...
	D.prototype.L = function(a) {
		if (a.eventKey == this.m) {
            ...
            this.ia.className = "gdx-display-block";            //LZY
			this.d.className = "gdx-display-none";
			this.ia.className = "gdx-bubble-unstar";
			if (a.meaningObj) {
				var b = a.meaningObj;
			...
			if (f) {
				...
				if (!s(b.target) && (f = e, k = "", window.getSelection ? (k = window.getSelection(), f = k.getRangeAt(0), k = m(k.toString())) : document.selection && (f = document.selection.createRange(), k = m(f.text)), k && !(1 == k.length && 127 >= k.charCodeAt(0) && !k.match(u)) && !("dblclick" == c && -1 != k.indexOf(" ")))) {
					...
					"false" == h.enableHttps && 0 == location.href.lastIndexOf("https", 0) ? K(a, t) : (window.setTimeout(r(a, function() {
						t == this.m && (this.h.innerHTML = "Searching...", this.b.className = "gdx-display-none", this.e.className = "gdx-display-none", this.i.className = "gdx-display-none", this.d.className = "gdx-display-none", document.documentElement.appendChild(this.c), J(this))
	...
	D.prototype.GA = function(a) {
		...
		b.onreadystatechange = function() {
			if (4 == b.readyState) try {
				document.getElementById(ia.id).className = "gdx-bubble-star";	//b.responseText
			} catch (a) {
				alert('catch');
			}
		};
